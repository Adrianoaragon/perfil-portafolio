...
export function renderBackground(background, dom) {
  if (background.type === 'color' || background.type === 'gradient') {
    dom.bgLayer.style.background = background.value;
  } else if (background.type === 'image') {
    dom.bgLayer.style.backgroundImage = `url(${background.value})`;
    dom.bgLayer.style.backgroundSize = 'cover';
    dom.bgLayer.style.backgroundPosition = 'center';
  }
  if (background.overlay) {
    dom.bgOverlay.style.display = 'block';
    dom.bgOverlay.style.background = `rgba(0,0,0,${background.overlayOpacity ?? 0.62})`;
  }
}
...
export function buildSocials(socials, dom) {
  ...
  socials.forEach((social) => {
    ...
    link.setAttribute('data-label', social.label || social.icon);
    link.innerHTML = ICONS[social.icon] || ICONS.link;
    dom.socialsRow.appendChild(link);
  });
}

export function renderMusicShell(music, dom) {
  if (!music?.enabled) return;
  dom.musicPlayer.classList.remove('hidden');
  dom.musicTitle.textContent = music.title || '';
  dom.musicArtist.textContent = music.artist || '';
  if (music.cover) dom.musicCover.src = music.cover;
}
...
export function renderActivity(activity, dom) {
  ...
  const largeUrl = getActivityImageUrl(activity);
  if (largeUrl) {
  ...
  const smallUrl = getActivitySmallUrl(activity);
  if (smallUrl) {
  ...
  const fallback = dom.activityCard.querySelector('.act-icon-fallback');
  if (!largeUrl) {
    if (!fallback) {
      const fb = document.createElement('div');
      fb.className = 'act-icon-fallback';
      fb.innerHTML = ICONS[ACT_ICON[activity.type] || 'link'];
      dom.actLarge.insertAdjacentElement('afterend', fb);
    }
  } else if (fallback) {
    fallback.remove();
  }
...
function getActivityImageUrl(activity) {
  ...
  if (largeKey.startsWith('spotify:')) return `https://i.scdn.co/image/${largeKey.replace('spotify:', '')}`;
  if (largeKey.startsWith('mp:external/')) return `https://media.discordapp.net/external/${largeKey.replace('mp:external/', '')}`;
  if (appId) return `https://cdn.discordapp.com/app-assets/${appId}/${largeKey}.png`;
  return null;
}

function getActivitySmallUrl(activity) {
  const appId = activity.application_id;
  const smallKey = activity.assets?.small_image;
  if (!smallKey) return null;
  if (smallKey.startsWith('mp:external/')) return `https://media.discordapp.net/external/${smallKey.replace('mp:external/', '')}`;
  if (appId) return `https://cdn.discordapp.com/app-assets/${appId}/${smallKey}.png`;
  return null;
}