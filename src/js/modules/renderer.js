...
const ICONS = {
  ...
  // Fallbacks para actividades sin imagen
  game:       `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M15 7.5V2H9v5.5l3 3 3-3zM7.5 9H2v6h5.5l3-3-3-3zM9 16.5V22h6v-5.5l-3-3-3 3zM16.5 9l-3 3 3 3H22V9h-5.5z"/></svg>`,
...
const ACT_FALLBACK_ICON  = { 0: 'game', 1: 'tv', 2: 'music_note', 3: 'tv', 4: 'link', 5: 'game' };
const ACT_TYPE_LABEL     = ['Jugando', 'Streaming', 'Escuchando', 'Viendo', 'Personalizando', 'Compitiendo'];

/* ── Background ── */
export function renderBackground(background, dom) {
  const { type, value, overlay, overlayOpacity } = background;

  if (type === 'color' || type === 'gradient') {
    dom.bgLayer.style.background = value;
  } else if (type === 'image') {
    dom.bgLayer.style.backgroundImage    = `url(${value})`;
    dom.bgLayer.style.backgroundSize     = 'cover';
    dom.bgLayer.style.backgroundPosition = 'center';
  } else if (type === 'video') {
    const v = document.createElement('video');
    Object.assign(v, { src: value, autoplay: true, loop: true, muted: true, playsInline: true });
    v.style.cssText = 'position:fixed;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;';
    document.body.prepend(v);
    dom.bgLayer.style.background = 'transparent';
  }

  if (overlay) {
    dom.bgOverlay.style.display    = 'block';
    dom.bgOverlay.style.background = `rgba(0,0,0,${overlayOpacity ?? 0.62})`;
  }
}

/* ── Perfil estático (fallback mientras carga Lanyard) ── */
export function renderStaticProfile(profile, dom) {
  if (!profile) return;
  if (profile.avatar)   dom.avatarImg.src = profile.avatar;
  if (profile.name)     dom.profileName.textContent = profile.name.toLowerCase();
  if (profile.bio)      dom.profileBio.textContent  = profile.bio.toUpperCase();
  if (profile.location) dom.profileLoc.textContent  = `📍 ${profile.location.toUpperCase()}`;
}

/* ── Aplicar datos de perfil de Lanyard ── */
export function renderDiscordProfile(profile, dom) {
  if (profile.avatar)  dom.avatarImg.src = profile.avatar;
  ...
  if (profile.decoUrl) {
    dom.avatarDeco.src = profile.decoUrl;
...
/* ── Redes sociales ── */
export function buildSocials(socials, dom) {
  dom.socialsRow.innerHTML = '';
  socials.forEach((s) => {
    ...
    dom.socialsRow.appendChild(a);
  });
}

/* ── Actividad de Discord ── */
export function renderActivity(activity, dom) {
  dom.activityCard.classList.remove('hidden');

  dom.actType.textContent   = ACT_TYPE_LABEL[activity.type] ?? 'Actividad';
  dom.actName.textContent   = activity.name    ?? '';
  dom.actDetail.textContent = activity.details ?? '';
  dom.actState.textContent  = activity.state   ?? '';

  // Imagen grande
  const largeUrl = _getActivityLargeUrl(activity);
  const wrap     = dom.actImgsWrap;

  // Quitar fallback previo
  wrap.querySelector('.act-icon-fallback')?.remove();

  if (largeUrl) {
    dom.actLarge.src          = largeUrl;
    dom.actLarge.style.display = 'block';
  } else {
    dom.actLarge.removeAttribute('src');
    dom.actLarge.style.display = 'none';
    // Mostrar icono SVG según tipo
    const fb = document.createElement('div');
    fb.className = 'act-icon-fallback';
    fb.innerHTML = ICONS[ACT_FALLBACK_ICON[activity.type] ?? 'link'];
    wrap.appendChild(fb);
  }

  // Imagen pequeña (app icon superpuesta)
  const smallUrl = _getActivitySmallUrl(activity);
  if (smallUrl) {
    dom.actSmall.src = smallUrl;
    dom.actSmall.classList.remove('hidden');
  } else {
    dom.actSmall.classList.add('hidden');
  }
}

/* ── Helpers de URLs de actividad ── */
function _getActivityLargeUrl(activity) {
  const appId    = activity.application_id;
  const largeKey = activity.assets?.large_image;
  if (!largeKey) return null;

  // Spotify: "spotify:ab67616d0000b273..."
  if (largeKey.startsWith('spotify:')) {
    return `https://i.scdn.co/image/${largeKey.replace('spotify:', '')}`;
  }
  // Medios externos (YouTube Music, etc.)
...
function _getActivitySmallUrl(activity) {
  const appId    = activity.application_id;
  const smallKey = activity.assets?.small_image;
